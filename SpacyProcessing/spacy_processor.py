...
def process_ingredient_list(ingredient_list):
    """Process a single recipe's ingredient list using NER (with noun-chunk fallback)
    and better splitting for combined ingredients like 'butter or margarine'."""

    # Parse JSON-like string into Python list if needed
    ...
    if ingredient_list:
        for ing in ingredient_list:
            ...
            if cleaned:
                # Split on commas to separate things like "butter, margarine"
                parts = [p.strip() for p in cleaned.split(',') if p.strip()]
                texts_to_process.extend(parts)

    if not texts_to_process:
        return recipe_ingredients

    # Run texts through spaCy in batches
    for doc in nlp.pipe(texts_to_process, batch_size=SPACY_BATCH_SIZE):
        # Prefer PRODUCT entities if available
        entities = [
            ent for ent in doc.ents
            if ent.label_ in {"PRODUCT"}
        ]

        # Fallback to noun chunks if NER finds nothing
        spans = entities if entities else list(doc.noun_chunks)
        ...
        for span in spans:
            ...
            for token in span:
                ...
                if (
                        txt not in noise_words
                        and not token.is_punct
                        and not token.is_space
                        and not token.is_stop
                ):
                    # Lemmatize nouns (incl. PROPN) so potato(s) → potato, onions → onion
